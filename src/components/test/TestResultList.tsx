...
const TestResultList = () => {
  ...
  <div className="flex justify-between items-center gap-3 lg:gap-12 w-full sm:w-auto">
    <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Test Results</h1>
            ...
            <Table className="min-w-[800px]">
              <TableHeader className="sticky top-0 z-10 bg-yellow-50 border-b">
                <TableRow className="hover:bg-yellow-50">
                  ...
                  <TableHead className="min-w-32 bg-yellow-50">Result</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {currentTestResults.length > 0 ? (
                  currentTestResults.map((course, index) => (
                    <TableRow key={index} className={index % 2 === 0 ? "bg-gray-50" : "bg-muted/30"}>
                      ...
                      <TableCell className="min-w-32 font-medium text-foreground">{course?.score}</TableCell>
                      <TableCell className="min-w-32 font-medium text-foreground">{course?.result}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
