...
const DynamicForm = ( { brandName, setBrandName, data, setData, onSubmit } : DynamicFormProps) => {
  ...
  const isModelComplete = (model: IModel) => {
    return (
      ...
      model.generations.every(
        (gen) =>
          isGenerationComplete(gen) && gen.engines.length > 0 && gen.engines.every((eng) => isEngineComplete(eng)),
      )
    )
  }

  const isFormValid = () => {
    return data.models.length > 0 && data.models.every((model) => isModelComplete(model))
  }
  ...
  const canAddEngine = (modelIndex: number, genIndex:number) => {
    ...
    return isEngineComplete(generation.engines[generation.engines.length - 1])
  }
  ...
  const handleInputChange = (modelIndex:number, genIndex:number | undefined, engineIndex:number | undefined, field:string, value:string) => {
    const newData = JSON.parse(JSON.stringify(data))
    if (engineIndex !== undefined && genIndex !==undefined) {
      newData.models[modelIndex].generations[genIndex].engines[engineIndex][field] = value
  ...
  const removeEngine = (modelIndex:number, genIndex:number, engineIndex:number) => {
    const newData = JSON.parse(JSON.stringify(data))
    newData.models[modelIndex].generations[genIndex].engines.splice(engineIndex, 1)
    setData(newData)
  }
      ...
      <div className="max-w-4xl mx-auto">
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          {data.models.map((model, modelIndex) => (
            <div key={modelIndex} className="bg-white rounded-lg shadow-md overflow-hidden border border-slate-200">
              {/* Model Header */}
              <div
                ...
                onClick={() => setExpandedModel(expandedModel === modelIndex ? -1 : modelIndex)}
              >
                <div className="flex items-center gap-3">
                  <span className={`transform transition-transform ${expandedModel === modelIndex ? "rotate-90" : ""}`}>
                    ▶
                  </span>
                  <h2 className="text-lg font-semibold">
                    Model {modelIndex + 1}: {model.modelName || "Untitled"}
                  </h2>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleInputChange(modelIndex, undefined, undefined, "modelName", e.target.value)}
                      className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
                  ...
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-slate-800">Generations</h3>
                    {model.generations.map((generation, genIndex) => (
                      <div key={genIndex} className="bg-slate-50 rounded-lg border border-slate-300 overflow-hidden">
                        {/* Generation Header */}
                        <div
                          ...
                          onClick={() =>
                            setExpandedGeneration({
                              ...expandedGeneration,
                              [`${modelIndex}-${genIndex}`]: !expandedGeneration[`${modelIndex}-${genIndex}`],
                            })
                          }
                        >
                          <div className="flex items-center gap-3">
                            <span
                              className={`transform transition-transform ${
                                expandedGeneration[`${modelIndex}-${genIndex}`] ? "rotate-90" : ""
                              }`}
                            >
                              ▶
                            </span>
                            <h4 className="font-semibold">
                              Generation {genIndex + 1}: {generation.generationName || "Untitled"}
                            </h4>
                          </div>
                          {model.generations.length > 1 && (
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                removeGeneration(modelIndex, genIndex)
                              }}
                              className="px-3 py-1 bg-red-600 hover:bg-red-700 rounded text-sm font-medium transition-colors"
                            >
                              Remove
                        ...
                        {/* Generation Content */}
                        {expandedGeneration[`${modelIndex}-${genIndex}`] && (
                          <div className="p-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                <input
                                  type="text"
                                  value={generation.generationName}
                                  onChange={(e) =>
                                    handleInputChange(modelIndex, genIndex, undefined, "generationName", e.target.value)
                                  }
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    handleInputChange(modelIndex, genIndex, undefined, "body", e.target.value)
                                  }
                                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
                              ...
                              <div>
                                ...
                                <input
                                  type="date"
                                  value={generation.productionStart}
                                  onChange={(e) =>
                                    handleInputChange(
                                      modelIndex,
                                      genIndex,
                              ...
                              <div>
                                ...
                                <input
                                  type="date"
                                  value={generation.productionEnd}
                                  onChange={(e) =>
                                    handleInputChange(modelIndex, genIndex, undefined, "productionEnd", e.target.value)
                                  }
                                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
                                />
                              </div>
                            ...
                            <div className="mt-6 pt-4 border-t border-slate-300">
                              <h5 className="font-semibold text-slate-800 mb-4">Engines (Required: At least 1)</h5>
                              <div className="space-y-3">
                                {generation.engines.map((engine, engineIndex) => (
                                  <div key={engineIndex} className="bg-white border border-slate-200 rounded-lg p-4">
                                    <div className="flex justify-between items-center mb-4">
                                      <h6 className="font-semibold text-slate-700">Engine {engineIndex + 1}</h6>
                                      {generation.engines.length > 1 && (
                                        <button
                                          type="button"
                                          onClick={() => removeEngine(modelIndex, genIndex, engineIndex)}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                      ...
                                      <div>
                                        <label className="block text-xs font-semibold text-slate-700 mb-1">KW *</label>
                                        <input
                                          type="number"
                                          value={engine.kw}
                                          onChange={(e) =>
                                      ...
                                      <div>
                                        <label className="block text-xs font-semibold text-slate-700 mb-1">HP *</label>
                                        <input
                                          type="number"
                                          value={engine.hp}
                                          onChange={(e) =>
                                      ...
                                      <div>
                                        <label className="block text-xs font-semibold text-slate-700 mb-1">CCM *</label>
                                        <input
                                          type="number"
                                          value={engine.ccm}
                                          onChange={(e) =>
                              ...
                              <button
                                type="button"
                                onClick={() => addEngine(modelIndex, genIndex)}
                                disabled={!canAddEngine(modelIndex, genIndex)}
                                className={`mt-4 w-full px-4 py-2 rounded-lg font-semibold transition-colors ${
                                  canAddEngine(modelIndex, genIndex)
